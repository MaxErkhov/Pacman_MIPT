...
public:
    int timer;
    Player(std::string F, float X, float Y, float W, float H):Characteristics(F, X, Y, W, H){  //Конструктор с параметрами(формальными) для класса Player. При создании объекта класса мы будем задавать имя файла, координату Х и У, ширину и высоту
    }
    ...
    void interactionWithMap()//ф-ция взаимодействия с картой
    {
        ...
        for (int i = y / 20; i < (y + h) / 20; i++)//проходимся по тайликам, контактирующим с игроком, то есть по всем квадратикам размера *20
            for (int j = x / 20; j<(x + w) / 20; j++)//икс делим на 20, тем самым получаем левый квадратик, с которым персонаж соприкасается. (он ведь больше размера 20*20, поэтому может одновременно стоять на нескольких квадратах). А j<(x + w) / 20 - условие ограничения координат по иксу. то есть координата самого правого квадрата, который соприкасается с персонажем. таким образом идем в цикле слева направо по иксу, проходя по от левого квадрата (соприкасающегося с героем), до правого квадрата (соприкасающегося с героем)
            {
                ...
                if (TileMap[i][j] == 's') { //если символ равен 's' (желтый кружок)
                    score += 50; // счет игрок, +50 за каждый кружок
                    TileMap[i][j] = ' ';//убираем кружок
                }
                if (TileMap[i][j] == 'f') { //если символ равен 'c' (камень)
...
int main()
{
    ...
    RenderWindow window(VideoMode(1460, 1000), "Huyacman!"); //создает игровое окно
...
//работа с игровый окном, пока открыто вызов функций и обновление игры по времени и клоку
    while (window.isOpen())
    {
        ...
        if(((p.x<=m.x&&m.x<=p.x+75)&&(p.y<=m.y&&m.y<=p.y+75))||((p.x>=m.x&&m.x>=p.x-75)&&(p.y>=m.y&&m.y>=p.y-75))||((p.x<=m.x&&m.x<=p.x+75)&&(p.y>=m.y&&m.y>=p.y-75))||((p.x>=m.x&&m.x>=p.x-75)&&(p.y<=m.y&&m.y<=p.y+75))){
            if(p.cherry==false&&(m.dx!=0||m.dy!=0)){//если коснулись монстра и нет ягодки, игрок умирает
                p.life = false;
            }
            else if(p.cherry == true){ //если коснулись монстра и есть вишенка, то умирает монстр - останавливаем и стираем спрайт
        ...
        if(((p.x<=m1.x&&m1.x<=p.x+75)&&(p.y<=m1.y&&m1.y<=p.y+75))||((p.x>=m1.x&&m1.x>=p.x-75)&&(p.y>=m1.y&&m1.y>=p.y-75))||((p.x<=m1.x&&m1.x<=p.x+75)&&(p.y>=m1.y&&m1.y>=p.y-75))||((p.x>=m1.x&&m1.x>=p.x-75)&&(p.y<=m1.y&&m1.y<=p.y+75))){
        ...
        if(p.life==true&&p.score!=7650){//описание управления игроком в случае если он жив и счет не м
            if ((Keyboard::isKeyPressed(Keyboard::Left) ||
                (Keyboard::isKeyPressed(Keyboard::A)))) { //движение влево
                counttime += time;
                if (counttime > 250) {
                    CurrentFrame += 1;
                    counttime = 0;
                ...
                p.dir = 1;
                p.speed = 0.1;//dir =1 - направление вверх, speed =0.1 - скорость движения. Заметьте - время мы уже здесь ни на что не умножаем и нигде не используем каждый разif (CurrentFrame > 3) CurrentFrame -= 3;
            ...
            if ((Keyboard::isKeyPressed(Keyboard::Right) ||
                (Keyboard::isKeyPressed(Keyboard::D)))) {//движение вправо
                counttime += time;
                if (counttime > 250) {
                    CurrentFrame += 1;
                    counttime = 0;
            ...
            if ((Keyboard::isKeyPressed(Keyboard::Up) ||
                (Keyboard::isKeyPressed(Keyboard::W)))) {//движение вверх
                counttime += time;
                if (counttime > 250) {
                    CurrentFrame += 1;
                    counttime = 0;
            ...
            if ((Keyboard::isKeyPressed(Keyboard::Down) ||
                 (Keyboard::isKeyPressed(Keyboard::S)))) { //движение вниз
                counttime += time;
                if (counttime > 250) {
                    CurrentFrame += 1;
                    counttime = 0;
        ...
        if(p.life == false){//завершение игры в случае смерти
            m1.dx = 0;
            m1.dy = 0;
            m.dx = 0;
            m.dy = 0;
        ...
        if(p.score == 7650){//завершение игры в случае набора нужного счета
            m1.dx = 0;
            m1.dy = 0;
            m.dx = 0;
            m.dy = 0;
